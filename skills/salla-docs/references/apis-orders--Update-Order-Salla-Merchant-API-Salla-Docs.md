# Apis Orders  Update Order Salla Merchant Api Salla Docs

## Table of Contents

- [apis-orders/Update-Order-Salla-Merchant-API-Salla-Docs](#apis-orders-update-order-salla-merchant-api-salla-docs)
...
paths:
  /orders/{order_id}:
    put:
      ...
      deprecated: false
      description: >-
        This endpoint allows you to update a specific order by passing the
        `order_id` as a path parameter. 


        :::tip[Tip]


        - For a [darfted order](https://docs.salla.dev/api-5750257?nav=1), you
        have no restrictions to update any field.

        - You can update the order based on the following five details:
        ...
        | Customer | It can be modified if there's no associated `customer` or
        if the customer is  `guest`. |

        | Receiver | It can be modified if the order status isn't marked as
        `completed`. |

        | Shipping | It can be modified if the order status isn't marked as
        `completed`. |

        | Payments | It can be modified if the order status is set to `pending`.
        |

        | Coupon | It can be modified if the order status isn't marked as
        `completed`. |
        ...
        <CardGroup cols={2}>
          <Card title="Introducing National Address" icon="material-two-tone-signpost">
            **Effective January 5th, 2026**, the [National Address](https://splonline.com.sa/en/national-address-1/) details are **mandatory** with the `ship_to` object as follows:
            - `country`
            - `city`
            - `address_line`
            - `street_number`
          ...
          <Card title="Deprecated variables" icon="material-two-tone-do_not_disturb">
          - The variables `data.checkout_url` and `data.rating_link` will be *deprecated*
        by **May 25, 2025**. Moving forward, they will be accessible under the
        `urls`

        object as `data.urls.checkout` and `data.urls.rating`.
          - The variables, `items.codes` and `items.files`, are deprecated. We recommend
        using instead the `data.urls.digital_content` variable.
          - The extended response is deprecated for all apps created after **15th of August,
        2024.** 
          </Card>
        </CardGroup>
              ...
              properties:
                customer:
                  ...
                  properties:
                    id:
                      type: integer
                      description: >-
                        Customer unique ID. List of Customer ID can be found
                    ...
                    mobile:
                      type: string
                      description: >-
                        The numerical contact information belonging to a
                        customer that allows communication via telephone,
                        without country code.
                    email:
                      type: string
                      description: >-
                        Email address of the customer used for electronic
                        communication.
                  x-apidog-orders:
                    - id
                    - name
                    - mobile
                    - email
                  description: 'Required if you are updating the `customer` details only '
                  ...
                  properties:
                    name:
                      ...
                      description: >-
                        ...
                        has been sent or delivered. It identifies the recipient
                        or destination for the shipped items.
                    country_code:
                      type: string
                      description: >-
                        A code that represents the specific destination country
                        to which a the order has been sent or is being
                        delivered.
                    phone:
                      type: string
                      description: >-
                        The phone number associated with the individual or
                        entity to whom a shipment has been sent or delivered.
                    email:
                      type: string
                      description: >-
                        The email address associated with the individual or
                        entity to whom a shipment has been sent or delivered.
                    notify:
                      type: boolean
                      description: >-
                        Whether or not to notify the receiver of the order
                        update
                  x-apidog-orders:
                    - name
                    - country_code
                    ...
                    - email
                    - notify
                  description: 'Required if you are updating the `receiver` details only '
                  x-apidog-ignore-properties: []
                delivery_method:
                  type: string
                  description: >-
                    Required if you are updating the `delivery_method` details
                    only 
                branch_id:
                  type: integer
                  description: >-
                    A unique identifier associated with a specific branch. List
                    of Branch ID  can be found
                ...
                ship_to:
                  ...
                  properties:
                    country:
                      type: integer
                      description: >-
                        Ship to destination country. Fetch a list of country
                        codes using [this](https://docs.salla.dev/5394228e0)
                        endpoint.
                    city:
                      type: integer
                    ...
                    district:
                      type: integer
                      description: >-
                        Ship to destination district. Fetch a list of districts
                    ...
                    short_address:
                      type: string
                      description: >-
                        A compact 8-character Saudi address code (4 letters + 4
                        digits), e.g., RHMA3184.
                    building_number:
                      type: string
                      description: >-
                        The National Address building number associated with the
                        location.This field is required only for addresses in
                        Saudi Arabia (KSA)
                    additional_number:
                      type: string
                      description: >-
                        An additional National Address identifier used for more
                        precise location specification.This field is required
                        only for addresses in Saudi Arabia (KSA).
                    address:
                      type: string
                      description: 'Shipment Ship To Address '
                    geo_coordinates:
                      type: object
                      properties:
                        lat:
                ...
                payment:
                  ...
                  properties:
                    ...
                    method:
                      type: string
                      description: >-
                        The way a customer chooses to pay for a product or
                        service.
                    store_bank_id:
                      type: integer
                      description: Bank ID of the store.
                    receipt_image_path:
                      type: string
                      description: The URL link of the receipt image
                    accepted_methods:
                      type: array
                      items:
                        type: string
                      description: Accepted methods of payments
                    cash_on_delivery:
                      type: object
                      properties:
                        amount:
                          type: integer
                          description: The amount for cash on delivery
                        currency:
                          type: string
                          description: The currency of the amount of the cash on delivery
                      x-apidog-orders:
                        - amount
                        - currency
                      x-apidog-ignore-properties: []
                  x-apidog-orders:
                    ...
                    - accepted_methods
                    - cash_on_delivery
                  description: 'Required if you are updating the `payment` details only '
                  x-apidog-ignore-properties: []
                coupon_code:
                  type: string
                  description: >-
                    Required if you are updating the `coupon_code` details only
                    . List of Coupon code can be found
                ...
                employees:
                  ...
                  items:
                    type: integer
                  description: >-
                    The employees' ID assigned to orders. List of employees' ID
                    can be found [here](https://docs.salla.dev/api-5394259)
              x-apidog-orders:
                - customer
                - receiver
                - delivery_method
      ...
      x-run-in-apidog: https://app.apidog.com/web/project/451700/apis/api-5751605-run
components:
  schemas:
    order_response_body:
      type: object
